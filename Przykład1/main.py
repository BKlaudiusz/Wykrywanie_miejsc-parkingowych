...
# Parametry startowe
place_width=25 # szerokoć miejsca parkingowego
thresh = 0.95  # prawdopodobieństwo od jakiego twierdzimy, że miejsce jest wolne
...
vertical = cv2.HoughLinesP(edges, 0.1, np.pi / 10, 15, 4, 1)
...
for x in range(0, len(vertical)):
    for x1, y1, x2, y2 in vertical[x]:
        ...
        if abs(x2 - x1) <=1  and 500 > abs(y2 - y1) >= 20 and x1>=10:
...
clus_dist =50  # odległość pomiędzy nowymi wierszami samochodów
...
for curr_parking_lot_row in wierszeparkingu:
    ...
    # wykresy
    f1 = plt.figure(1)
    plt.subplot('61%d' % (curr_idx + 1)), plt.plot(curr_parking_lot_row.col_mean), plt.title(
        'Wiersz %d korelacja z wolnym miejscem' % (curr_idx + 1))
    
    f2 = plt.figure(2)
    plt.subplot('61%d' % (curr_idx + 1)), plt.plot(curr_parking_lot_row.inverted_variance), plt.title(
        'Wiersz %d wariacja' % (curr_idx + 1))

    f3 = plt.figure(3)
    plt.subplot('61%d' % (curr_idx + 1))
